import type { Metadata } from "next";
import { redirect } from "next/navigation";
import ProposalContent from "@/components/proposals/purpose-capital-managed-it-e8e515";
import { hasProposalAccess } from "@/lib/proposals/access";

const proposalSlug = "purpose-capital-managed-it-e8e515";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Purpose Capital — Managed IT, Security & AI Readiness Proposal | ITECS",
  robots: {
    index: false,
    follow: false,
    noarchive: true,
    nosnippet: true,
    noimageindex: true,
  },
};

export default async function ProposalPage() {
  if (!(await hasProposalAccess(proposalSlug))) {
    redirect(`/p/${proposalSlug}/access`);
  }

  return (
    <div className="min-h-screen">
      <ProposalContent />
    </div>
  );
}
