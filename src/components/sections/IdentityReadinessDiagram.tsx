"use client";

import { motion } from "framer-motion";
import { ClipboardList, Split, FileCheck, LogOut } from "lucide-react";
import { ScrollReveal } from "@/components/effects/ScrollReveal";

const steps = [
  {
    icon: ClipboardList,
    name: "Inventory",
    detail:
      "List which apps offer ChatGPT sign-in and who would use it.",
  },
  {
    icon: Split,
    name: "Separate",
    detail:
      "Split authentication from any connector that can read or change data.",
  },
  {
    icon: FileCheck,
    name: "Consent",
    detail:
      "Verify and document what profile data each consent screen shares.",
  },
  {
    icon: LogOut,
    name: "Offboard",
    detail:
      "Define a disconnect and revocation path before anyone signs in.",
  },
];

export function IdentityReadinessDiagram() {
  return (
    <section className="not-prose py-4">
      <figure
        role="img"
        aria-label="A four-phase path to Sign in with ChatGPT identity readiness. Phase one, Inventory: list which apps offer ChatGPT sign-in and who would use it. Phase two, Separate: split authentication from any connector that can read or change data. Phase three, Consent: verify and document what profile data each consent screen shares. Phase four, Offboard: define a disconnect and revocation path before anyone signs in."
      >
        <ol className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, i) => {
            const Icon = step.icon;
            return (
              <ScrollReveal key={step.name} delay={i * 0.1}>
                <motion.li
                  className="relative flex h-full flex-col chamfer-md border border-[var(--card-line)] bg-card p-5"
                  whileHover={{
                    borderColor: "var(--itecs-blue)",
                    transition: { duration: 0.2 },
                  }}
                >
                  <span
                    className="absolute left-0 top-0 h-0.5 bg-itecs-blue-bright"
                    style={{ width: `${25 + i * 25}%` }}
                    aria-hidden="true"
                  />
                  <div className="mb-3 flex items-center gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center chamfer-sm bg-brand-subtle text-itecs-blue">
                      <Icon className="h-5 w-5" aria-hidden="true" />
                    </span>
                    <div className="flex items-baseline gap-2">
                      <span className="font-mono text-xs text-itecs-blue">
                        0{i + 1}
                      </span>
                      <h3 className="font-display text-base font-semibold text-ink">
                        {step.name}
                      </h3>
                    </div>
                  </div>
                  <p className="text-sm leading-relaxed text-ink-muted">
                    {step.detail}
                  </p>
                </motion.li>
              </ScrollReveal>
            );
          })}
        </ol>
        <figcaption className="mt-5 text-center text-sm italic text-ink-muted">
          Inventory, separate, consent, then offboard — so ChatGPT sign-in stays
          a controlled identity choice, not a side door. ITECS builds and runs it
          with your team.
        </figcaption>
      </figure>
    </section>
  );
}
