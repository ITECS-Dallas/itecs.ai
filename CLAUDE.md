...
- **`src/components/intelligence-os/` + `src/lib/intelligence/`** — on-demand ITECS Intelligence OS, deterministic incident/configurator, and the server-owned, public-only knowledge and guardrail layer for its live AI Terminal. See `docs/intelligence-os.md`. Keep the terminal hidden on `/p/*`, keep model credentials server-side, and never add proposals or illustrative dashboards to its factual grounding.
